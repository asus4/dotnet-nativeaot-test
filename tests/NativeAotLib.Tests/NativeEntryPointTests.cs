using NativeAotLib.Core;

namespace NativeAotLib.Tests;

// Tests the managed *Core logic behind the [UnmanagedCallersOnly] entry points directly,
...
public class NativeEntryPointTests
{
    ...
    {
        Assert.Equal(expected, NativeEntryPoint.AddCore(a, b));
    }

    [Fact]
    public void WriteLineCore_ReturnsZero_ForNonEmptyString()
    {
        Assert.Equal(0, NativeEntryPoint.WriteLineCore("hello"));
    }
    ...
    {
        Assert.Equal(-1, NativeEntryPoint.WriteLineCore(input));
    }

    [Fact]
    public void SumStringCore_Concatenates()
    {
        Assert.Equal("foobar", NativeEntryPoint.SumStringCore("foo", "bar"));
    }
    ...
    {
        Assert.Null(NativeEntryPoint.SumStringCore(first, second));
    }
    ...
    {
        var result = await NativeEntryPoint.HttpGetAsync("https://example.com");
